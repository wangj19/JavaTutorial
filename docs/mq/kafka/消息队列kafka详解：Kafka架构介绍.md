...
**流程可以看成一条线：** Producer 把消息发给某个 Topic → 消息真正落在该 Topic 的某一个 Partition（分区）上 → 分区对应一段不断追加的 log → Consumer 按自己记录的 offset 从 log 里接着读。
...
**落盘结构：** 一个分区在磁盘上主要是一对对 `.index` + `.log`。`.log` 里按顺序追加真实消息；`.index` 里记「稀疏的 offset → 在 log 里的位置」，用来加速查找，而不是每条消息都建一个索引项。
...
**副本怎么摊在机器上：** 同一个分区的数据会在不同 Broker 上各放一份。例如分区 `third-2` 在 kafka01 上有一份，kafka03 上还有一份副本。一个分区可以设多个副本，其中 **一个 Leader、其余 Follower**；生产者写、消费者读都面向 Leader，Follower 只负责从 Leader 同步，用来备份和故障切换。
...
文件名是该分段的起始 offset。上面三对文件表示：从 offset 0 开始的一段、从 170410 开始的一段、从 239430 开始的一段。

**此时如何快速定位数据，步骤：**

`.index` 文件存储的是消息的 offset 与在 `.log` 里的相对位置；`.log` 中存放的是真实的数据。

查找某条消息时：先按分段起始 offset **二分**，落到对应的那一对 index/log（例如要找的 offset 落在 0～170409 之间，就打开 `00000000000000000000.*`）。再在 `.index` 里 seek 到最接近的索引项，用「分段起始 offset + 索引给出的文件内位移」定位到 `.log` 里的字节位置，从而读出这条消息，而不必从头扫整个大文件。
...
**可靠发送的顺序：** Producer 把消息交给该分区的 Leader → Leader 写入本地 log → Follower 从 Leader 拉取并写入自己的 log → 达到约定的同步范围后，Leader 才给 Producer 回 ack → Producer 才认为这次发送成功，否则重发。
...
**ack=1 丢数据的典型路径：** Leader 已落盘并回了 ack，Follower 还没跟上；此时 Leader 宕机，新 Leader 从旧 Follower 里选，那条只存在于已挂 Leader 上的消息就没了，而 Producer 以为已经成功。
...
**ack=-1 重复的典型路径：** ISR 都已写上，但 ack 还没回到 Producer；Leader 挂了，Producer 超时重发，新 Leader 上就会再落一条同样的业务数据。所以 -1 是「至少一次」，不是精确一次；精确一次要靠后文幂等/事务。

producer返ack，0无落盘直接返，1只leader落盘然后返，-1全部落盘然后返

## 六. 数据一致性问题

每个副本自己有一个 **LEO（Log End Offset）**：该副本 log 里下一条要写入的位置，也就是目前最后一条的下一位。**HW（High Watermark，高水位）** 是消费者目前能读到的最大 offset，取 ISR 里各副本 LEO 的最小值。HW 之后的数据副本间可能还没对齐，所以不让消费者看见，避免读到稍后会被截掉的脏尾。
...
**Round-Robin 在转什么：** 把所有分区排成一列，消费者也排成一列，分区依次问「当前这个 Consumer 订了这个 Topic 吗」——订了就分给它，没订就跳到下一个 Consumer，分区本身不丢，继续往下问。结果往往是订阅面最广的人（例子里的 C2）分到最多分区，压力不均。
...
**Range 在转什么：** 不把所有 Topic 的分区混在一起轮询，而是 **每个 Topic 单独切一刀**。例如 t0 的三个分区、两个订阅者，按名字排序后前一半（含除不尽多出来的）给 C0，剩下给 C1；再对 t1 做同样的事。所以同一个 Consumer 在每个 Topic 上都可能分到「靠前」的那几片，多个 Topic 时也容易一头偏沉。

## 8.2 消费者offset的存储

由于Consumer在消费过程中可能会出现断电宕机等故障，Consumer恢复以后，需要从故障前的位置继续消费，所以Consumer需要实时记录自己消费到了那个offset，以便故障恢复后继续消费。

**记在哪：** 进度不是记在业务自己的数据库里（也可以，但默认不是）。Kafka 0.9 之前默认写在 **ZooKeeper**；0.9 之后默认写进集群内部 Topic **`__consumer_offsets`**。消费者组、Topic、分区、当前 offset 作为一条提交记录落在那里，重启后读回来从该 offset 接着拉。
...
**传统读路径（四次拷贝）：** 磁盘 → 内核页缓存 → 用户态缓冲区 → 内核 socket 缓冲区 → 网卡，再发给消费者。每多一个消费者，后面这几步往往要再走一遍。
...
**零拷贝：** 数据进页缓存之后，不再进用户态兜一圈，由内核直接把页缓存里的内容发给网卡。多个订阅者可以共用同一份页缓存。
...
Broker 在 ZooKeeper 上注册；集群选出一个 **Controller**。某分区 Leader 挂了之后，由 Controller 看该分区的 ISR，从中指定新 Leader，并把结果写回 ZooKeeper / 通知其他 Broker。Follower 按新 Leader 截 HW 之后的 log 再同步。选举依赖 ZK 上的集群元数据，不是消费者自己投票。
...
为了管理Transaction，Kafka引入了一个新的组件Transaction Coordinator，Producer就是通过有和Transaction Coordinator交互获得Transaction ID对应的任务状态，Transaction Coordinator还负责将事务信息写入内部的一个 Topic中，这样即使整个服务重启，由于事务状态得到保存，进行中的事务状态可以恢复，从而继续进行。
...
[https://blog.csdn.net/cao131502](https://blog.csdn.net/cao131502)
[https://zhuanlan.zhihu.com/p/137811719](https://zhuanlan.zhihu.com/p/137811719)
